...
const FavCard = ({FavCard, handleDelete, handleRemove}) => {
   // console.log(FavCard)
   return (
      <div className='mt-6'>
  ...
  <div class="ml-4 space-y-2 ">
      <div class="flex justify-between items-center ">
         <div>
       <h1 className="text-1xl font-bold line-clamp-1 ">{FavCard.title}</h1>
    <h2 class="text-xl font-semibold">${FavCard.currentBidPrice}</h2>
    <h3 class="text-lg text-gray-600">Bids: {FavCard.bidsCount}</h3>
   </div>
   <div className='flex items-center '>
      <button onClick={()=>{
          handleDelete(FavCard.id);
          handleRemove(FavCard.currentBidPrice);
      }
